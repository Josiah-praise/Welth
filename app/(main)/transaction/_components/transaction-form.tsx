...
import { TransactionSchema, transactionSchema} from "@/schema/transactionSchema";
...
export function AddTransactionForm({
  ...
  const {
    ...
    editMode
      ? updateTransaction({ id: editId!, data: data as TransactionSchema})
      : createTransaction(data as TransactionSchema)
  );

    type TransactionFormData = addTransactionProps["initialData"] & {
        amount: string;
    }
  ...
  const handleScanComplete = (scannedData: ScannedData) => {
    ...
    // }
    setValue("amount", '40000');
    setValue("date", new Date());
    setValue("description", 'She said i will not understand');
            ...
            <SelectContent>
              {accounts.map((account) => (
                <SelectItem key={account.id} value={account.id}>
                  {account.name} (&#8358;
                  {parseFloat(account.balance as unknown as string).toFixed(2)})
                </SelectItem>
              ))}
              <CreateAccountDrawer>
                <Button
